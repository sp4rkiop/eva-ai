...
class Subscriptions(Base):
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'model_id', name='uix_user_model'),
    )

    sub_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, server_default=text("gen_random_uuid()"))
    ...
    model_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("ai_models.model_id", ondelete="CASCADE"), index=True, nullable=False)
