...
class UserService:
    ...
    async def get_create_user(
        ...
        """
        logger.info(
            f"Getting or creating user for email: {email_id}, partner: {partner}"
        )
        async with PostgreSQLDatabase.get_session() as session:
            try:
                # Try to get existing user
                stmt = select(Users).where(
                    Users.email == email_id, Users.partner == partner
                )
                ...
                new_sub = Subscriptions(
                    user_id=new_user.user_id, model_id=default_model_id
                )
                session.add(new_sub)

                token = await self._generate_jwt_token(new_user.user_id, new_user.role)
                # Build response before commit to include all data
                response = {"user_id": new_user.user_id, "token": token}
                await session.commit()

                logger.info(
                    f"New user created for email: {email_id}, partner: {partner}"
                )
                return response
                ...
                logger.error(
                    f"Failed to create user due to database constraints: {ex}",
                    exc_info=True,
                )
                raise ValueError(
    ...
    async def _update_user_session_index(
        self, user_id: uuid.UUID, cookie_token: str
    ) -> bool:
        """
    ...
    async def _generate_jwt_token(self, user_id: uuid.UUID, role: str) -> str:
        ...
        # Store session data
        await self.redis.set(name=jti, value="", ex=self.SESSION_EXPIRY)
    ...
    async def get_single_conversation(
        ...
        try:
            async with PostgreSQLDatabase.get_session() as session:
                ...
                stmt = (
                    select(
                        ChatHistory, UserDocument.document_id, UserDocument.file_name
                    )
                ...
                return {
                    ...
                    "conversation": pickle.loads(chat.history_blob)
                    if chat.history_blob
                    else [],
    ...
    async def get_subscribed_models(self, user_id: uuid.UUID) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Log the exception
            logger.exception(
                f"Failed to get subscribed models: {str(ex)}", exc_info=True
            )
            raise Exception(f"Failed to get subscribed models: {str(ex)}")

    async def is_model_subscribed(
        ...
        try:
            ...
            if is_subscribed:
                await self.redis.set(
                    cache_key, int(is_subscribed), 14400
                )  # Cache for 4 hours
        ...
        try:
            async with PostgreSQLDatabase.get_session() as session:
                update_stmt = (
                    update(ChatHistory)
                    .where(
                        ChatHistory.chat_id == chat_id, ChatHistory.user_id == user_id
                    )
    ...
    async def delete_conversation(self, user_id: uuid.UUID, chat_id: uuid.UUID) -> bool:
        ...
        try:
            async with PostgreSQLDatabase.get_session() as session:
                ...
                delete_stmt = delete(UserDocument).where(
