...
async def login_signup(
    signup_request: AuthRequest, user_service: UserService = Depends(get_user_service)
):
    try:
...
async def get_single_conversation(
    ...
    try:
        result = await user_service.get_single_conversation(
            uuid.UUID(payload["user_id"]), uuid.UUID(chat_id)
        )
        return result
...
async def rename_or_delete_conversation(
    ...
    try:
        if title:
            result = await user_service.rename_conversation(
                uuid.UUID(payload["user_id"]), uuid.UUID(chat_id), title
            )
            if result:
                return Response(status_code=204)
        elif delete_request:
            result = await user_service.delete_conversation(
                uuid.UUID(payload["user_id"]), uuid.UUID(chat_id)
            )
            if result:
                return Response(status_code=204)
